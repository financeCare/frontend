...
class _BudgetPerMonthScreenState extends State<BudgetPerMonthScreen> {
                ...
                onPressed: () {
                  ...
                  if (amount != null && amount > 0) {
                    final transaction = TransactionRequest(
                      categoryId: int.parse(_budgetItems[index].categoryId),
                      amount: amount,
                    ...
                    createTransaction(transaction)
                        .then((_) {
                          ...
                          _loadBudgetData(); // Refresh UI from server (fetches updated 'spent' amount)
                          Navigator.pop(context);
                        })
                        .catchError((error) {
                          ScaffoldMessenger.of(context).showSnackBar(
  ...
  void _showEditBudgetDialog(int index) {
    ...
    final initialValue = item.limitBudget % 1 == 0
        ? item.limitBudget.toInt().toString()
        : item.limitBudget.toString();
        ...
        return AlertDialog(
          ...
          actions: [
            ...
            ElevatedButton(
              style: ElevatedButton.styleFrom(backgroundColor: Colors.green),
              onPressed: () {
                final newBudgetCode = controller.text;
                final newBudget = double.tryParse(newBudgetCode);
                if (newBudget != null) {
                  // Use budgetId if available, fallback to categoryId
                  final idToUpdate = item.budgetId ?? item.categoryId;

                  if (idToUpdate.isNotEmpty) {
                    _budgetService
                        .updateBudgetLimit(idToUpdate, newBudget)
                        .then((_) {
                          ScaffoldMessenger.of(context).showSnackBar(
                            const SnackBar(
                              content: Text('อัปเดตงบประมาณสำเร็จ'),
                            ),
                          );
                          _loadBudgetData(); // Refresh UI from server
                          Navigator.pop(context);
                        })
                        .catchError((error) {
                          ScaffoldMessenger.of(context).showSnackBar(
                    ...
                    // This part is fallback for local update if both IDs are missing
                    setState(() {
                      _budgetItems[index] = _budgetItems[index].copyWith(
                        limitBudget: newBudget,
                      );
                    });
                    Navigator.pop(context);
                  }
